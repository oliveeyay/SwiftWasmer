...
use wasmer::{
    ...
    BaseTunables, Engine, MemoryType, TableType, Target, Tunables,
};
...
"#
    ...
    let tunables = DynamicTunables::new(base);
...
pub struct DynamicTunables<T: Tunables> {
    /// The base implementation we delegate all the logic to
    base: T,
}

impl<T: Tunables> DynamicTunables<T> {
    pub fn new(base: T) -> Self {
        Self { base }
    }
}

impl<T: Tunables> Tunables for DynamicTunables<T> {
    /// Forces the dynamic `MemoryStyle`
    fn memory_style(&self, _memory: &MemoryType) -> MemoryStyle {
        println!("Forcing dynamic memory style");
        MemoryStyle::Dynamic {
            offset_guard_size: 0x2_0000,
        }
    }
    ...
    unsafe fn create_vm_memory(&self, ty: &MemoryType, style: &MemoryStyle, vm_definition_location: NonNull<VMMemoryDefinition>) -> Result<vm::VMMemory, MemoryError> {
    ...
    unsafe fn create_vm_table(&self, ty: &TableType, style: &TableStyle, vm_definition_location: NonNull<VMTableDefinition>) -> Result<vm::VMTable, String> {
